...
app.get("/queue", (req,res) => {
    ...
    db.any(getAllUsers)
        .then((data) => {
            ...
            // console.table(data);
            data.forEach((person) => {users.push(person.person)})
...
app.get('/select-self', (req,res) => {
    ...
    const getFriendNamesIDs = 'select person, id from users u inner join relations r on (u.id = r.user2) where r.user1 = (select id from users where person = \''+req.query["self-dropdown"]+'\');';
    var friends = [];
    ...
    //        const selfId = data[0].id;
    //        const friendsQuery = 'select * from relations where user1 = \''+selfId+'\';';
    //        // console.log('friendsQuery: ' + friendsQuery);
    //        db.any(friendsQuery)
    //            .then((data) => {

    //                var friendIDs = [];
    //                console.log("Friends from person selected in dropdown");
    //                console.table(data);

    //                data.forEach((row) => {
    //                    friendIDs.push(row.user2);
    //                })
    ...
    //                console.error(err);
    //                res.render('queue', {message: 'messed up getting friends in /select-self'})

    //            })

    //        //then, get all my friends & render it
    //    })
    //    .catch((err) => {

    //        console.error("Problem getting self in /select-self")
    //        res.render('queue', {message: 'messed up in /select-self'});
